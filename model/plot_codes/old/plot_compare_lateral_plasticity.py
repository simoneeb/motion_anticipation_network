...
outname =  f'/user/sebert/home/Documents/Simulations/motion/anticipation_1D/selma/bipolar_pooling_lateral_plastic/compare_fixed_plastic'
label1 = 'lateral inhibtion w = -60'
label2 = '+plasticity in Amacrines '


# define what to compare
net_name = 'sselma/bipolar_pooling_lateral_plastic/'
params_name = 'betaA'
...
# define what to compare
net_name = 'selma/selma_net_bipolar_pooling_lateral_plasticity_A'
params_name = 'initial'
stim_name = 'smooth_2.1'
fp = f'/user/sebert/home/Documents/Simulations/motion/anticipation_1D/{net_name}/{params_name}/{stim_name}'
save = True
...
ax[0].set_ylabel('V(t)')
ax[1].set_ylabel('n(t)')
ax[2].set_ylabel('I(t) into Amacrines')
ax[3].set_ylabel('R(t)')
ax[0].set_title('Bipolar Responses', loc = 'left')
ax[1].set_title('Occupancy', loc = 'left')
ax[3].set_title('GC Firing Rate', loc = 'left')
...
for i,c in enumerate(BC_cells):
    ...
    #plotter1.plot_one_BC(0,c,ax[0],'', response='RB', alpha = 0.5, color = scalarMap_pooling.to_rgba(BC_cells_weight[i]))
    plotter2.plot_one_BC(0,c,ax[0], alpha = 1, color = scalarMap_gaincontrol.to_rgba(BC_cells_weight[i]), label = f'{c}')
    ax[1].plot(params['time'],out['res'][0]['n'][c], alpha = 1, color = scalarMap_gaincontrol.to_rgba(BC_cells_weight[i]), label = f'{c}')
    ax[2].plot(params['time'],out['res'][0]['n'][c]*out['res'][0]['X'][c], alpha = 1, color = scalarMap_gaincontrol.to_rgba(BC_cells_weight[i]), label = f'{c}')


#plotter1.plot_one_GC(CELL_GC,ax[1], label1, alpha = 1, color = 'grey', linewidth = 2)
plotter2.plot_one_GC(CELL_GC,ax[3], label2, alpha = 1, color = scalarMap_gaincontrol.to_rgba(BC_cells_weight.max()), linewidth = 2)

ax[0].legend()
ax[1].legend()

if save == True :
    fig.savefig(f'{outname}_n.png')
