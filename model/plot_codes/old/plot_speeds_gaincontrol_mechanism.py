...
ax110 = fig.add_subplot(gs[1,3])
ax111 = fig.add_subplot(gs[1,4])
ax112 = fig.add_subplot(gs[1,5])
...
for i,s in enumerate(speeds):
    ...
    ax1.set_title('Bipolar Activity', loc = 'left', fontsize = fontsize_labels)
    anti,maxi = plotter2.plot_one_BC(CELL = CELL_GC,ax = ax1, color=cmap_activity(i), layer = 0, label = f'v = {s} mm/s', response = 'AB', middlecrossing_at_0=True)

    maxis_act.append(maxi)
    ax112.scatter(s,GainF(maxi), color = cmap_gaincontrol(i), s = 100)
    ax110.scatter(s, maxi, color = cmap_gaincontrol(i), s = 100)
    ax111.scatter(maxi,GainF(maxi), color = cmap_gaincontrol(i), s = 100)

    ax2.set_title('Bipolar Response after Gain Control', loc = 'left', fontsize = fontsize_labels)
...
ax110.set_title('Max Activity', fontsize = fontsize_labels, loc = 'left')
ax111.set_title('Gain Function', fontsize = fontsize_labels, loc = 'left')
ax112.set_title('Max Gain ', fontsize = fontsize_labels, loc = 'left')


range =np.arange(-0.2,3,0.1)
ax111.plot(range,[GainF(m) for m in range],color = 'k')
...
ax110.set_ylabel('Act(v)',fontsize = fontsize_labels)
ax111.set_ylabel('Gain(t)',fontsize = fontsize_labels)
ax112.set_ylabel('Gain(v)',fontsize = fontsize_labels)
...
ax110.set_xlabel('velocity [mum/s]',fontsize = fontsize_labels)
ax111.set_xlabel('A(t)',fontsize = fontsize_labels)
ax112.set_xlabel('velocity [mum/s]',fontsize = fontsize_labels)
