...
class plotting(object):
    ...
    def plot_one_BC(self,layer,CELL,ax, label, alpha = 0.5,response = 'RB', color = 'k', middlecrossing_at_0 = False):
        ...
        if middlecrossing_at_0 : 
                item = ax.plot(self.time-self.tps_rf_mid[CELL],r, label = label, alpha = alpha, color = color)
                ax.axvline(0,linestyle = ':', color = item[0].get_color(), alpha = alpha)

        else: 
                item = ax.plot(self.time,r, label = label, alpha = alpha, color = color)
                ax.axvline(self.tps_rf_mid[CELL],linestyle = ':', color = item[0].get_color(), alpha = alpha)
    ...
    def plot_one_GC(self,CELL, ax,label = '',y = 'time',response = 'RG', alpha = 1, linestyle = '-', color = 'k', linewidth = 3,middlecrossing_at_0 = False):
        ...
        if y == 'time':
            if middlecrossing_at_0 == True:
                item = ax.plot(self.time - self.tps_rf_GC_mid[CELL],self.out[response][CELL], label = label, alpha = alpha, color = color, linestyle = linestyle, linewidth = linewidth)
                ax.axvline(0,linestyle = ':', color = item[0].get_color(), alpha = 1, linewidth = linewidth) 

            else:
                item = ax.plot(self.time ,self.out[response][CELL], label = label, alpha = alpha, color = color, linestyle = linestyle, linewidth = linewidth)
    ...
    def plot_one_stim(self,CELL,ax):
        ...
        ax.plot(self.time,self.out['inp'][CELL,:], label = f'speed {self.speed}')
