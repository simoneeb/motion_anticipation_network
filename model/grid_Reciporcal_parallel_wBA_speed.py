...
net_name = f'Reciporcal_fitted_linear/noGCGainControl'

stim_type = 'smooth'

#loop over parameter
vals = np.arange(0,200,1)
#vals =[20,10,0.0]
...
df = pd.DataFrame(columns=['wBA', 'speed', 'peak_RG','peak_RB', 'peak_drive', 'tp_rf_GC_mid', 'peak_RG_pooling', 'peak_RB_pooling', 'onset_RB', 'onset_RG'])
...
def run(val1,si):
    ...
    params = make_params(param_names = ['speed','wBA'], param_vals=[si,val1])
